...
const SingleProduct = () => {
  // getting the single selected product
  const product = useSelector((state) => state.products.singleProduct);
  // getting the size of product
  const productSize = product[0].size ? product[0].size[0] : "";
  // getting the color of the product
  const productColor = product[0].color[0];
  // state of the product color
  const [color, setColor] = useState(productColor);
  // useDispatch hook
  const dispatch = useDispatch();
  // state of the product size
  const [size, setSize] = useState(productSize);
  // getting the ID of the product
  const { id } = useParams();

  // adding product to basket fucntion
  ...
  return (
    <div>
      <Link to="/">
        <button className="bg-[var(--primary-dark)] p-3 m-3 rounded-md border">
          Back to Home
        </button>
      </Link>
      {/* filtering and rendering the products */}
      {product
